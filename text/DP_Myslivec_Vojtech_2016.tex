...
V~této práci přinášíme ucelený pohled na asymetrický kryptosystém
\emph{McEliece}. Tento kryptosystém je založený na samoopravných lineárních
kódech a~je jedním z~kandidátů pro asymetrickou postkvantovou kryptografii.
V~práci uvádíme základní definici tohoto kryptosystému, příbuzný kryptosystém
\emph{Niederreiter}, variantu pro digitální podpis a~věnujeme se též existujícím
kryptoanalýzám a~praktickým aspektům tohoto systému. V~rámci práce vznikla
ukázková implementace v~softwaru Wolfram Mathematica.
...
s~kvazi-cyklickými \emph{MDPC} kódy se vyskytla v~draftu z~roku 2016 společnosti
\emph{IEEE} mezi doporučenými \emph{postkvantovými} asymetrickými
...
Poslední kapitola~\ref{kap_implementace} se zabývá ukázkovou implementací tohoto
kryptosystému v~softwaru \emph{Wolfram Mathematica} a~měřením časových
...
\begin{enumerate}
    \item Zvolíme \emph{lineární kód}\footnote{
        ...
        algoritmus) s~odpovídající $k \times n$ \emph{generující mati\-cí}~$G$.
...
jak bylo popsáno v~příloze~\ref{app_linearni_kody}.
...
\emph{Mathematica}, zvolenou implementaci a~popíšeme implementované algoritmy.
...
Použitý git -- ve větvi měření \url{...}. Na Přiloženém disku je umístěno
