...
(* ::Text:: *)
(*Overovano pouze pro praci s telesy GF(q^n) s charakteristikou 2! Tedy GF(2^m^n^...)*)


(* ::Section:: *)
(*konecnaTelesa -- implementace*)
...
doListu[ a_, x_Symbol, n_Integer ] := Module[ {
        list, la
    },
...
(*
Polynom nad polynomy (GF(p^n1^n2^...). ff je konecne teleso reprezentovano seznamem { n1, n2, ... }
List ff odpovida hodnote Dimensions
*)
nulovyPolynom[ p_Integer /; p >= 2, ff_List ] := Module[ {
        r = First[ ff]
    },
    If[ Length[ff] == 1,
        nulovyPolynom[ p, r ],
...
(* Polynom nad polynomy (GF(p^n1^n2^...). ff je konecne teleso reprezentovano seznamem { n1, n2, ... }
List ff odpovida hodnote Dimensions *)
jednotkovyPolynom[ p_Integer /; p >= 2, ff_List ] := Module[ {
        r = First[ ff]
    },
    If[ Length[ff] == 1,
...
(* Polynom nad polynomy (GF(p^n1^n2^...). ff je konecne teleso reprezentovano seznamem { n1, n2, ... }
List ff odpovida hodnote Dimensions *)
nahodnyPolynom[ p_Integer /; p >= 2, ff_List ] := Module[ {
        r = First[ff]
    },
    If[ Length[ff] == 1,
        nahodnyPolynom[ p, r ],
...
(* Polynom nad polynomy (GF(p^n1^n2^...). ff je konecne teleso reprezentovano seznamem { n1, n2, ... }
List ff odpovida hodnote Dimensions *)
nahodnyPolynomStupne[ p_Integer /; p >= 2, ff_List ] := Module[ {
        r = First[ff]
    },
    If[ Length[ff] == 1,
...
ireducibilniPolynom[ ff_Integer /; ff >= 2, r_Integer /; r >= 2 ] := Module[ {
        b = False, poly, x
    },
    While[ b == False,
...
plus[ a_List, b_List, p_Integer /; p >= 2 ] := Module[ {
...
redukujPolynom[
        ...
        M : { g_List, p_Integer } /; ArrayDepth[g] == 1 && p >= 2
    ] := Module[ {
        ll, x, ax, gx
    },
...
redukujPolynom[
        a_List /; ArrayDepth[a] > 1,
        M : { gVnejsi_List, MVnitrni_List } /; ArrayDepth[gVnejsi] >1 && Length[MVnitrni] == 2 && ArrayDepth[gVnejsi] == ArrayDepth[MVnitrni[[1]]]+1
    ] := Module[ {
    ...
    (* prevedeni g na monicky polynom -- if kvuli zrychleni pro pripad, ze uz monicky je *)
...
krat[
    ...
    (* postupne nasobeni pres koeficienty (od nejnizsich radu -> odzadu) *)
    For[ i = la, i>0, i--,
        If[ ! a[[i]] === nula,
    ...
    (* vrati vysledek redukovany modulem *)
    redukujPolynom[ cc, M ]
]
...
(*Vyuziva toho, ze inverze je umocnovani na 2^m-2 = (111...10)*)
(*m-1 rika na "kolik jednicek" se ma umocnit a nakonec jeste jednou na druhou.*)
...
inverzeITT[
    ...
    For[ i = 2, i <= Length[ rozvoj ], i++,
        ...
        If [ rozvoj[[i]] == 1,
            (* pricte jeste jednu jednicku do exponentu neboli *)
...
inverze[
    ...
    While[ stupenB != 0,
        ...
        (* podmineny vypis *)
        If[ vypis,
            Print[ Grid[ { {podil, C, tC }}, Frame->All, Alignment->Right, Spacings->4 ] ]
        ];
...
umocni[
        a_List,
        n_Integer,
        M : { gVnejsi_List, MVnitrni_ } /; (ArrayDepth[gVnejsi] == 1 && ArrayDepth[MVnitrni] == 0) || (ArrayDepth[gVnejsi] == ArrayDepth[MVnitrni[[1]]]+1)
    ] := Module[ {
        p = NestWhile[ Last, M, ListQ ],
...
(* ::Text:: *)
(*Odmocnina -- velmi experimentalni, nepouziva se*)
(*Mocneni na 1/r -- pro 2. odmocninu je tak mozne pouzit odmocni[a,2,M]*)
...
odmocni[
    ...
    (* rad telesa (v pripade ireducibilniho polynomu) *)
...
dlouheDeleni[
        polyP_List /; ArrayDepth[polyP] > 1,
        polyQ_List /; ArrayDepth[polyQ] > 1,
        M : { gVnitrni_List, MVnitrni_ } /; (ArrayDepth[gVnitrni] == 1 && ArrayDepth[MVnitrni] == 0) || (ArrayDepth[gVnitrni] == ArrayDepth[MVnitrni[[1]]]+1)
    ] := Module[ {
        p = NestWhile[ Last, M, ListQ ],
    ...
    (* cim se bude delit -- prvni nenulovy koef. *)
...
(* ::Text:: *)
(*Neni implementovana peclive -- je to jen fce krat[] namapovana na prvky polynomu. Chyby vyhodi az vnitrni funkce krat*)
